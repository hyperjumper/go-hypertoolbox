...
// DjaroDistance distance between two words is the minimum number
// of single-character transpositions required to change one word
// into the other.
func (sd *StringDiff) DjaroDistance() float32 {
	s := []byte(sd.S1)
	t := []byte(sd.S2)
...
// DjaroWinklerDistance uses a prefix scale which gives more
...
func (sd *StringDiff) DjaroWinklerDistance(p float32) float32 {
	a := []byte(sd.S1)
	b := []byte(sd.S2)
	dj := sd.DjaroDistance()
	sim := 0
